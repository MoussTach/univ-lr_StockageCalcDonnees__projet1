...
const AnanasAbstraction = artifacts.require('Ananas');
let HST;

contract('Ananas', (accounts) => {
  beforeEach(async () => {
    HST = await AnanasAbstraction.new(10000, 'Ananas', 1, 'ANA', { from: accounts[0] });
  });
  ...
  it('creation: test correct setting of vanity information', async () => {
    ...
    assert.strictEqual(name, 'Ananas');

    const decimals = await HST.decimals.call();
    assert.strictEqual(decimals.toNumber(), 1);

    const symbol = await HST.symbol.call();
    assert.strictEqual(symbol, 'ANA');
  });

  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const HST2 = await AnanasAbstraction.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'Ananas', 1, 'ANA', { from: accounts[0] });
